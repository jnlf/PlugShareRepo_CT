...
import { PlaywrightCrawler,Dataset,Sitemap,Configuration } from 'crawlee';
import fs from "fs";
/*
...
const crawler = new PlaywrightCrawler({
    ...
    minConcurrency: 1, //how many workers running at the same time (minimum)
    maxConcurrency: 2, //how many workers running at the same time (maximum)
    maxRequestsPerMinute: 10, //number of requests to the website to make to the website in a minute
    //maxRequestsPerMinute: Math.floor(getRandomDecimal(8,15)), #randomize requests on start
        launchContext: {
          launchOptions: {
    ...
    async requestHandler({waitForSelector, request, page}) {
      ...
      //PlugCount
      await page.locator("span.plug-count.ng-binding").locator('nth=-1').waitFor({state:"attached"});
      //PlugType
      ...
      //PlugCount
      const PlugCount = await page.locator("span.plug-count.ng-binding").allTextContents();;
      //PlugType
...
const urls = fs.readFileSync('/home/void/Desktop/Research/PlugShareCrawleeProject/FullCTRT.txt',
...
//await new Promise(r => setTimeout(r, (Math.floor((getRandomDecimal(2,5)*1000))))); // if running batch text files, wait a random second interval between
